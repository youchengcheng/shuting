...
public class FollowingCountShardingXxlJob {
    ...
    public void followingCountShardingJobHandler() throws Exception {
        ...
        for (;;){
            ...
            List<Long> userIds = selectMapper
                    .selectBatchFromDataAlignFollowingCountTempTable(tableNameSuffix, batchSize);
            //为空就结束循环
            if(Objects.isNull(userIds)) break;

            // 2: 循环这一批发生变更的用户 ID
            userIds.forEach(userId -> {
                //对 t_following 关注表执行 count(*) 操作，获取总数
