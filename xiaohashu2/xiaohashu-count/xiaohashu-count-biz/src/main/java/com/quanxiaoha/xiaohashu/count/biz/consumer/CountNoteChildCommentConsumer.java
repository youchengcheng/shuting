...
public class CountNoteChildCommentConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        Map<Long, List<CountPublishCommentMqDTO>> groupMap = countPublishCommentMqDTOList.stream()
                .filter(countPublishCommentMqDTO -> Objects.equals(CommentLevelEnum.TWO.getCode(), countPublishCommentMqDTO.getLevel()))
        ...
        for (Map.Entry<Long, List<CountPublishCommentMqDTO>> entry : groupMap.entrySet()) {
            ...
            if(hasKey) {
                //累加
                redisTemplate.opsForHash().increment(commentCountHashKey,RedisKeyConstants.FIELD_CHILD_COMMENT_TOTAL,count);
            }
        ...
        //获取字典汇中的所有评论id----keyset用于获取map中的所有key并封装为set集合返回
