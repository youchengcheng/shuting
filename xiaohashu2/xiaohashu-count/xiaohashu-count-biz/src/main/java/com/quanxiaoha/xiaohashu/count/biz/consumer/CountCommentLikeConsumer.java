...
@RocketMQMessageListener(
        consumerGroup = "xiaohashu_group_count_" + MQConstants.TOPIC_COMMENT_LIKE_OR_UNLIKE,
        ...
        for (Map.Entry<Long, List<CountLikeUnlikeCommentMqDTO>> entry : groupMap.entrySet()) {
            ...
            for (CountLikeUnlikeCommentMqDTO countLikeUnlikeCommentMqDTO : list) {
                ...
                switch (likeUnlikeCommentTypeEnum) {
                    case LIKE -> {//如果为点赞操作，点赞数 + 1
                        finalCount += 1;
                    }
                    case UNLIKE -> {//如果为取消点操作，点赞数 - 1
                        finalCount -= 1;
                    }
                }

                //将分组后投机出的最终计数，存入countList中
            ...
            countList.forEach(item -> {
                ...
                //redis中评论计数hash key
                String countCommentRdisKey = RedisKeyConstants.buildCountCommentKey(commentId);
                //判断redis中hash是否存在
                Boolean hasKey = redisTemplate.hasKey(countCommentRdisKey);

                // 若存在才会更新
                // (因为缓存设有过期时间，考虑到过期后，缓存会被删除，这里需要判断一下，存在才会去更新，而初始化工作放在查询计数来做)
                if(hasKey) {
                    //对目标用户hash中的点赞数字段进行计数操作
                    redisTemplate.opsForHash().increment(countCommentRdisKey,RedisKeyConstants.FIELD_LIKE_TOTAL,count);
                }
            });
