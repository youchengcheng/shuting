...
public class LikeUnlikeComment2DBConsumer {
    ...
    public DefaultMQPushConsumer mqPushConsumer() throws MQClientException {
        ...
        //创建一个新的 DefaultMQPushConsumer 实例，并指定消费者的消费者组名
        ...
        //注册消息监听器
        //MessageListenerOrderly：顺序消息监听者，msgs有顺序的消息，context上下文内容，如上方的一些consumer信息
        consumer.registerMessageListener((MessageListenerOrderly) (msgs,context) -> {
            log.info("==> 【评论点赞、取消点赞】本批次消息大小: {}", msgs.size());
            try {
                //令牌桶流控，以控制数据库能够承受的QPS
                rateLimiter.acquire();
                ...
                msgs.forEach(msg -> {
                    String tag = msg.getTags();//tag标签
                    String msgJson = new String(msg.getBody());// 消息体json字符串
                ...
                commentIdAndListMap.forEach((commentId,ops) -> {
                    ...
                    Map<Long, LikeUnlikeCommentMqDTO> userLastOp = ops.stream().collect(Collectors
                            .toMap(LikeUnlikeCommentMqDTO::getUserId, //以发布评论的用户id作为map键
                                    Function.identity(),//直接使用 DTO 对象本身作为map的值
                                    (oldValue, newValue) ->// 合并策略：当出现重复键（同一用户多次操作）时，保留时间更晚的记录
    ...
    * 批量操作数据库
    * */
    private void executeBatchSQL(Collection<LikeUnlikeCommentMqDTO> finalLikeUnlikeCommentMqDTOS) {
        //过滤出点赞操作
        List<LikeUnlikeCommentMqDTO> likes = finalLikeUnlikeCommentMqDTOS.stream()
                .filter(op -> Objects.equals(op.getType(), LikeUnlikeCommentTypeEnum.LIKE.getCode()))
                .toList();

        //过滤出取消点赞操作
        List<LikeUnlikeCommentMqDTO> unlikes = finalLikeUnlikeCommentMqDTOS.stream()
